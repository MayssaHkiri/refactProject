...
public class AccountingService {
  ...
  private ArrayList<Bill> findBills(Function<Bill, LocalDate> dateSelector, LocalDate from,
                                    LocalDate to) {
    return bills.stream()
  ...
  private boolean isInRange(LocalDate dateToCheck, LocalDate from, LocalDate to) {
    return dateToCheck != null &&
        (dateToCheck.isAfter(from) || dateToCheck.isEqual(from)) &&
        (dateToCheck.isBefore(to) || dateToCheck.isEqual(to));
  }
}
