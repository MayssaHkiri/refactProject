...
class inlineTemps {
  ...
  void deserveDiscount() {
    Order orderDeservingDiscount = Order.builder().products(new ArrayList<>(
        List.of(Product.builder().name("Something expensive").price(1000.01).build()))).build();
    Order orderNotDeservingDiscount = Order.builder().products(new ArrayList<>(
        List.of(Product.builder().name("Something less expensive").price(1000).build()))).build();
  ...
  void deserveDiscountBasedOnCustomer() {
    ...
    Order orderDeservingDiscount =
        Order.builder().customer(customer).products(generateFakeProducts(12)).build();
    Order orderNotDeservingDiscount =
        Order.builder().customer(customer).products(generateFakeProducts(9)).build();
